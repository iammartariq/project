...
import { sliderClass, defaultClass  } from './audio-player/helpers/styles/audio';
...
// import './audio-player/components/audioPlay.css';
type SVGConfig = {
  className: string;
  ...
  line1?: React.SVGProps<SVGLineElement>;
  line2?: React.SVGProps<SVGLineElement>;
};
...
export const AudioPlayer = forwardRef<AudioPlayerRef | undefined, AudioInterface>(
  (
    ...
    const getClassNames = () => {
      if(classNames === 'slider'){
        return sliderClass;
      }
      return defaultClass;
    };

    const handlePlayClassNames = (premium) => {
      if(isPlaying){
        return getClassNames().rapPpButton.pause.svg as SVGConfig;
      }else if(subscriptionStatus !== "active" && premium && !item.preview){
        return getClassNames().rapPpButton.lock.svg as SVGConfig;
      }else{
        return getClassNames().rapPpButton.play.svg as SVGConfig;
      }
    };
    ...
    return (
      ...
      >
        <div className={getClassNames().rapContainer.main2}>
          {hasError && (
          ...
          {canPlay && !hasError && (
            <div className={`rap-pp-button ${getClassNames().rapPpButton.main}`} onClick={() => togglePlay()}>
              <svg xmlns="http://www.w3.org/2000/svg" className={`rap-pp-icon ${handlePlayClassNames(item.premium).className}`} {...handlePlayClassNames(item.premium).props} >
                {handlePlayClassNames(item.premium).path && <path {...handlePlayClassNames(item.premium).path!}></path>}
                {handlePlayClassNames(item.premium).circle && <circle {...handlePlayClassNames(item.premium).circle!}></circle>}
                {handlePlayClassNames(item.premium).rect && <rect {...handlePlayClassNames(item.premium).rect!}></rect>}
                {handlePlayClassNames(item.premium).polygon && <polygon {...handlePlayClassNames(item.premium).polygon!}></polygon>}
                {handlePlayClassNames(item.premium).line1 && <line {...handlePlayClassNames(item.premium).line1!}></line>}
                {handlePlayClassNames(item.premium).line2 && <line {...handlePlayClassNames(item.premium).line2!}></line>}
              </svg>
            </div>
          )}
          <div>
              <div className={getClassNames().rapTexts.album}>{item?.album?.title}</div>
              <div className={getClassNames().rapTexts.title}>{handleTitle()}</div>
          </div>
        </div>
